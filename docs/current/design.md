...
**한 줄 정의:**
시드 문서와 질문을 하나의 `수조`에 넣으면, BettaFish가 현실의 재료를 정리하고 MiroFish 계열 시뮬레이션이 여러 `해류`와 `물고기`의 상호작용을 통해 가능한 미래의 `멀티버스`를 탐색한 뒤, 언어에 맞는 보고서와 인터랙티브 세계를 돌려주는 예측/시뮬레이션 워크스페이스.

**디자인 원칙:**
- **단계형은 유지, 형태는 재해석:** MiroFish의 “처음 사용자도 따라가기 쉬운 5단계 흐름”은 차용하되, 숫자 리스트/콘솔 복제 대신 `수조 준비 → 생태계 조성 → 해류 가동 → 멀티버스 관찰 → 리포트/대화`라는 은유적 진행으로 차별화한다.
...
- Trust note: `BettaFish 보고서와 일반 문서를 모두 지원합니다.`
